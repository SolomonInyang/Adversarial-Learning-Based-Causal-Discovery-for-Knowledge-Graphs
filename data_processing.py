...
def read_causal_pairs(filename, scale=False, **kwargs):
    ...
    Examples:
        cdt.utils import read_causal_pairs
        read_causal_pairs('file.tsv', scale=True, sep='\\t')
    """
...
def read_adjacency_matrix(filename, directed=True, **kwargs):
    ...
    Examples:
        cdt.utils import read_adjacency_matrix
        read_causal_pairs('graph_file.csv', directed=False)
    """
    if isinstance(filename, str):
        data = read_csv(filename, **kwargs)
    elif isinstance(filename, DataFrame):
